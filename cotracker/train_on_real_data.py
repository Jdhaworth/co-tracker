...
from cotracker.datasets.custom_dataset import CustomDataset
    ...
    def run(self, args):
        ...
        if self.global_rank == 0:
            ...
            for ds_name in args.eval_datasets:
                eval_dataset_path = './data/tapvid/tapvid_davis/tapvid_davis.pkl'
                eval_dataloaders.append(
                    (ds_name, get_eval_dataloader(eval_dataset_path, ds_name))
                )
            final_dataloaders = eval_dataloaders  # Only use specified eval datasets
        ...
        model = build_cotracker(args.restore_ckpt).cuda()
        ...
        #teacher_models = []
        teacher_models = [
            ("online_cotracker_three", CoTrackerThreeOnline().cuda()),
            ("tapir", TAPIRPredictor().cuda())
        ]  # Use only two teacher models to reduce memory
        #if args.model_name == "cotracker_three":
        #    teacher_models.append(("online_cotracker_three", CoTrackerThreeOnline().cuda()))
        #    teacher_models.append(("offline_cotracker_three", CoTrackerThreeOffline().cuda()))
        #    teacher_models.append(("tapir", TAPIRPredictor().cuda()))
        #else:
        #    teacher_models.append(("cotracker", CoTracker2().cuda()))

        evaluator = Evaluator(exp_dir=args.ckpt_path)
        visualizer = Visualizer(save_dir=args.ckpt_path)
        logger = Logger(model, scheduler, args.ckpt_path)
        save_freq = args.save_freq

        if args.validate_at_start:
            if self.global_rank == 0:
                run_test_eval(evaluator, model, eval_dataloaders, logger.writer, 0)
                model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluate_every_n_epoch", type=int, default=1, help="evaluate during training after every n epochs")
    parser.add_argument("--save_every_n_epoch", type=int, default=1, help="save checkpoints during training after every n epochs")
    parser.add_argument("--validate_at_start", action="store_true", help="whether to run evaluation before training starts")
    parser.add_argument("--save_freq", type=int, default=100, help="frequency of trajectory visualization during training")
    parser.add_argument("--traj_per_sample", type=int, default=768, help="the number of trajectories to sample for training")
    parser.add_argument("--dataset_root", type=str, help="path to all the datasets (train and eval)")
    parser.add_argument("--train_iters", type=int, default=4, help="number of updates to the disparity field in each forward pass.")
    parser.add_argument("--sequence_len", type=int, default=8, help="train sequence length")
    parser.add_argument("--eval_datasets", nargs="+", default=["tapvid_davis_first"], help="what datasets to use for evaluation")
    parser.add_argument("--num_virtual_tracks", type=int, default=None, help="stride of the CoTracker feature network")
    parser.add_argument("--dont_use_augs", action="store_true", help="don't apply augmentations during training")
    parser.add_argument("--sample_vis_1st_frame", action="store_true", help="only sample trajectories with points visible on the first frame")
    parser.add_argument("--sliding_window_len", type=int, default=8, help="length of the CoTracker sliding window")
    parser.add_argument("--model_stride", type=int, default=8, help="stride of the CoTracker feature network")
    parser.add_argument("--crop_size", type=int, nargs="+", default=[384, 512], help="crop videos to this resolution during training")
    parser.add_argument("--eval_max_seq_len", type=int, default=1000, help="maximum length of evaluation videos")
    parser.add_argument("--debug", action="store_true", help="saves launch time for faster debug")
    parser.add_argument("--random_frame_rate", action="store_true", help="random_frame_rate")
    parser.add_argument("--real_data_splits", type=int, nargs="+", default=[0], help="real data folders")
    parser.add_argument("--loss_only_for_visible_pts", action="store_true", help="compute sequence loss only for visible points")
    parser.add_argument("--real_data_filter_sift", action="store_true", help="select point to track based on SIFT features")
    parser.add_argument("--train_grid_size", type=int, default=5, help="number of extra regular grid points that we sample at training")
    parser.add_argument("--train_sift_size", type=int, default=0, help="number of extra SIFT points that we sample at training.")
    parser.add_argument("--real_data_filter_superpoint", action="store_true", help="select point to track based on SuperPoint features")
    parser.add_argument("--train_only_visible_points", action="store_true", help="Loss only for visible points")
    parser.add_argument("--offline_model", action="store_true", help="training the offline model")
    parser.add_argument("--clean_kubric", action="store_true", help="filtering out bad tracks in Kubric")
    parser.add_argument("--random_number_traj", action="store_true", help="when training on Kubric, sampling a random number of tracks")
    parser.add_argument("--random_seq_len", action="store_true", help="when training on Kubric, cropping the sequence")
    parser.add_argument("--uniform_query_sampling_method", action="store_true", help="Whether to sample points uniformly across time")
    parser.add_argument("--limit_samples", type=int, default=10000, help="limit samples on real data")
